...
where I: Clone + 'static, O: Clone + 'static
{
    prefix: Rc<Box<dyn Parser<I, Box<dyn UnOp<O>>>>>,
    postfix: Rc<Box<dyn Parser<I, Box<dyn UnOp<O>>>>>,
    infix: Rc<Box<dyn Parser<I, Box<dyn BinOp<O>>>>>,
    infix_l: Rc<Box<dyn Parser<I, Box<dyn BinOp<O>>>>>,
    infix_r: Rc<Box<dyn Parser<I, Box<dyn BinOp<O>>>>>,
}
...
fn min_level<I, O>(mix: MixfixParser<I, O>) -> Option<usize> {
    mix.levels.keys().min().cloned()
}

fn next_level<I, O>(mix: MixfixParser, prev: usize) -> Option<usize> {
    mix.levels.keys().filter(|curr| *curr > &prev).min().cloned()
}

fn terminals_parser(mix: MixfixParser) -> Rc<dyn Parser<I, O>> {
    let terminals = mix.terminals.clone();
    Rc::new(
...
#[cutlass::curry]
pub fn parse(input: I) -> IResult<I, O> {
    algo::mainloop(min_level())(input)
}

mod algo {

    pub fn mainloop(current: Option<usize>) -> Rc<dyn Parser<I, O>> {
        match current {
            None => {
            ...
            Some(curr) => {
                ...
                build_level_directed(
                    curr_level.clone(),
                    build_level_nondirected(
                        curr_level.clone(),
                        mainloop(self.next_level(curr))
                    )
                )
            }
        }
